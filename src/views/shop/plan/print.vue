    ...
    <div id="printList">
      <div class="order-list" v-for="(list, index) in printList" style="page-break-after: always">
        ...
        <div class="order-info">
          下单日期: {{formatTimeTwo(list.putDate)}}
        </div>
        <div class="order-info">
          订单号: {{list.orderId}}
        </div>
      ...
      <el-table border show-summary :data="[list]"  :summary-method="getSummaries" size="small" style="width: 100%;">
        <el-table-column type="index" label="行号" :index="indexMethod" width="60mm"/>

        <el-table-column prop="productId" label="商品">
          <template slot-scope="scope">
            <span>{{ scope.row.storeName}} / {{ scope.row.sku}} / {{ scope.row.putCount}} </span>
          </template>
        </el-table-column>
    ...
    getSummaries(param) {
      ...
      columns.forEach((column, index) => {
        if (index === 0) {
          ...
          const values = data.map(item => Number(item.putCount));
