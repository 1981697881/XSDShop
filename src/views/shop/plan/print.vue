  ...
  <el-dialog id="printDialog" :append-to-body="false" :close-on-click-modal="false" :before-close="cancel" :visible.sync="dialog" title="订单打印" width="220mm">
    ...
    <div id="printList">
      ...
      <div class="order-info">
        ...
        <span class="info">下单日期 : {{formatTimeTwo(list.putDate)}}</span>
        <span>客户名称 : {{list.realName}}</span>
      </div>
      <div class="order-info">
        <span class="info">联系人 : {{list.realName}}</span>
        <span class="info">联系电话 : {{list.userPhone}}</span>
        <span>收货地址 : {{list.userAddress}}</span>
      </div>
      </div>
      <el-table border show-summary :data="printList"  :summary-method="getSummaries" size="small" style="width: 100%;">
        <el-table-column type="index" label="行号" :index="indexMethod" width="60mm"/>

        <el-table-column prop="productId" width="80mm" label="商品编号" />
        ...
        <el-table-column prop="price" width="80mm" label="单价" />

        <el-table-column prop="putCount" width="80mm" label="数量" />

        <el-table-column prop="cartInfoMap" width="80mm" label="小计(元)" >
          <template slot-scope="scope">
            <span>{{ scope.row.price*scope.row.putCount }}</span>
          </template>
        </el-table-column>
